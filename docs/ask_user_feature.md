...
✅ **Блокирует тики** пока ждёт ответа — агент не продолжает работу, пока пользователь не ответит

## Блокировка тиков

**ВАЖНО:** Когда `awaiting_answer` установлен, агент **останавливает обработку** до получения ответа:
...
Это предотвращает:
- ❌ Агент задаёт несколько вопросов подряд
- ❌ Агент делает другую работу пока ждёт ответа
- ❌ Агент забывает что задал вопрос
